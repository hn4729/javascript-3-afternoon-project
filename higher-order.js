...
//Code Here
let totalPopulation = populations.reduce((total, current) => total + current);
...
let orderTotals = orders.map(ordersObj => ordersObj.price * (ordersObj.tax + 1));
...
let bobsTotal = purchases.filter(purchaseObj => purchaseObj.owner === "Bob").map(bobPurchases => bobPurchases.price).reduce((total, current) => total + current);
